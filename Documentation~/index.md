...
- The scriptable object just needs to exist within the project. DO NOT make more than one. Only the first one found will
  be used.
...
- Set `Enabled` to true.
- Import your animations in the `TargetFolder` specified in the Settings SO. Default: `Assets/_Project/Animations` (or a
  subfolder).
- They should now have the same settings applied to them as your example animation as well as the specified settings in
  the Settings SO.
...
- `AutoSingleton<T>:`: Regular singleton that generates an instance on access if none is found.
- `AUtoPersistentSingleton<T>:`: A generic singleton that persists between scenes and generates an instance on access if
  none is found.
...
- **VisualElement Extensions**:
    - VisualElement FindElementByName(this VisualElement element, string name)
    - VisualElement FindElementByTooltip(this VisualElement element, string tooltip)
